...
const textFiles = ['To The Lighthouse - Virginia Woolf.txt',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('keydown', (event) => {
    if (yesButton.style.display === 'inline-block') {
      if (event.key === 'y') {
        handleUserInput('y');
      } else if (event.key === 'n') {
        handleUserInput('n');
  ...
  function startRound() {
    ...
    fetch(`${randomFile}`)
      ...
      .then(contents => {
        ...
        const hiddenSentences = consecutiveSentences.map(sentence => sentence.replace(randomFile, '__________'));

        // Display the hidden sentences
        hiddenSentencesElement.textContent = 'Guess the book from these three consecutive sentences:\n' + hiddenSentences.join('. ');

        // Show the Yes, No, and Skip buttons
  ...
  function handleUserInput(input) {
    ...
    // Update the DOM elements with the context
    ...
    // Start a new round
    setTimeout(startRound, 1); // Adjust the time (inmilliseconds) as needed
}
